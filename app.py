# app.py — Media Impacts: Quick ITS (centered form, minimal inputs)
# Primary UI: Project title + Date of release (centered).
# Advanced options contain measurement source, outcome topic, Trends/Wiki settings,
# date comparison, batch mode, and optional visuals.
...
def fetch_trends_weekly(queries: List[str], geo: str, timeframe: str = "today 5-y",
    ...
    """Google Trends weekly series (W-MON), averaged across up to 5 queries. Manual retry."""
...
def wiki_weekly_pageviews(title_or_query: str, start: str, end: str, lang: str = "en") -> pd.Series:
    ...
    if not _summary_exists(title):
        ...
        if not hits: raise RuntimeError(f"No Wikipedia page found for “{title_or_query}”.")
        title = hits[0]["title"]
    ...
    if not items: raise RuntimeError(f"No pageview data for “{title}”.")
...
def quick_its(outcome: pd.Series, film: pd.Series, intervention_date: str):
    ...
    df = pd.concat({"outcome": outcome.rename("outcome"),
                    "film_interest": film.rename("film_interest")}, axis=1).dropna()
    if len(df) < 12:
    ...
    X = sm.add_constant(df[["time","post","time_post","film_lag1"]]); y = df["outcome"]
        ...
        with right, st.spinner("Fetching data and running ITS…"):
            ...
            with st.expander("What these charts mean (plain language)", expanded=True):
                ...
                readable = (
                    ...
                    f"**Source:** {'Google Trends' if 'Google' in data_source else 'Wikipedia pageviews'}.{regions_text}\n"
            ...
            plt.bar(df.index[after_mask], excess[after_mask]); plt.axvline(metrics["t0_monday"], linestyle="--")
            plt.title("Weekly Excess vs Counterfactual"); plt.tight_layout(); st.pyplot(fig)
            st.caption("Weekly lift above (or below) baseline after the intervention.")

            cum_excess = excess.cumsum()
            fig = plt.figure(figsize=(10,3))
            plt.plot(df.index, cum_excess); plt.axvline(metrics["t0_monday"], linestyle="--")
            plt.title("Cumulative Excess Interest"); plt.tight_layout(); st.pyplot(fig)
            ...
            es = df.loc[window, ["outcome"]].copy(); es["k"] = weeks_from[window]
            ...
            plt.xlabel("Weeks from intervention"); plt.tight_layout(); st.pyplot(fig)
            ...
            if n_placebo >= 10:
                ...
                for d0 in picks:
                    res = _fit_at(d0)
                    if "post" in res.params: placebo_effects.append(float(res.params["post"]))
                if placebo_effects:
                    fig = plt.figure(figsize=(10,3))
            ...
            if "Google" in data_source and show_regions:
                try:
                    ...
                    plt.barh(top["region"][::-1], top[qcol][::-1]); plt.title("Where interest is highest")
...
if 'batch_file' in locals() and batch_file is not None:
    ...
    try:
        bdf = pd.read_csv(batch_file); rows=[]
